...
encodingTests =
  ...
  , let
    ...
    in
      (denorm == p) <?> "Normalize/Denormalize round-trip failed"
  ...
  , let
    ...
    in
      (decoded == Just { puzzle: norm, key: k }) <?> "Encode/Decode round-trip failed"
  ...
  , let
    ...
    in
      (decodePuzzle Sudoku (repeat 91 '0') == Nothing) <?> "decodePuzzle long failed"

  , let
      repeat n c = foldr (\_ s -> s <> singleton c) "" (1 .. n)
    in
      (decodePuzzle Sudoku (repeat 81 'a' <> repeat 9 'b') == Nothing) <?> "decodePuzzle non-digit failed"

  , let
      repeat n c = foldr (\_ s -> s <> singleton c) "" (1 .. n)
    in
      (decodePuzzle Wordoku (repeat 81 '0' <> "112345678") == Nothing) <?> "decodePuzzle duplicate key failed"

  , let
      repeat n c = foldr (\_ s -> s <> singleton c) "" (1 .. n)
    in
      (decodePuzzle Wordoku (repeat 81 '0' <> "123.45678") == Nothing) <?> "decodePuzzle dot in key failed"
  ]
