...
data Route
  ...
  | PuzzleRoute Game Difficulty String DecodedKey Int
...
instance showRoute :: Show Route where
  ...
  show (PuzzleRoute g d p k n) = "PuzzleRoute " <> show g <> " " <> show d <> " " <> p <> " " <> show k <> " " <> show n
    ...
    case segments of
        ...
        case parseGame gStr, parseDifficulty dStr of
          Just g, Just d ->
            case decodePuzzle g pStr of
              Just { puzzle, key } -> PuzzleRoute g d puzzle key 0
      ...
      [ gStr, dStr, pStr, nStr ] ->
        case parseGame gStr, parseDifficulty dStr of
          Just g, Just d ->
            case decodePuzzle g pStr of
              Just { puzzle, key } -> PuzzleRoute g d puzzle key (parseClueCount nStr)
              _ -> DifficultyRoute g d
          Just g, Nothing -> GameRoute g
          _, _ -> Home
      _ -> Home

-- | Clamps malformed/negative values to `0` silently. Upper-bound clamping
-- | against the puzzle's blank count happens later in
-- | `Sudoku.Display.applyClues`, which is the only place that knows it.
parseClueCount :: String -> Int
parseClueCount s = case Int.fromString s of
  Just n | n >= 0 -> n
  _ -> 0

buildPath :: Game -> Difficulty -> String -> DecodedKey -> Int -> String
buildPath g d p k n =
